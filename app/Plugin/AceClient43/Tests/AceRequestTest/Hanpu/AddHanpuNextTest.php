...
use Plugin\AceClient43\AceServices\Model\Request\Hanpu\AddHanpuNext;
...
class AddHanpuNextTest extends AceRequestTestAbtract
{
    private ?string $testMemberId = '221';
    private ?string $testSessid = '8888888888';

    public function getAddHanpuNextModel(): AddHanpuNext\AddHanpuNextRequestModel
    {
        $member = (new AddHanpuNext\MemberModel())
                        ->setJmember((new AddHanpuNext\JmemberModel())->setCode($this->testMemberId))
                        ->setSmember((new AddHanpuNext\SmemberModel())->setCode($this->testMemberId))
                        ->setNmember((new AddHanpuNext\NmemberModel())->setEda(1));

        $handen = (new AddHanpuNext\HandenModel())
                        ...
                        ->setBscode(100)
                        ->setPointm(0)
                        ->setCardInfo((new AddHanpuNext\CardInfoModel())
                                            ->setCcode(100)
                                            ->setCno('123456789')
                        ...
                        ->setHanpucd('21')
                        ->setHcnt(1)
                        ->setHanpuFirst((new AddHanpuNext\HanpuFirstModel())
                                ->setSday((new \DateTime())->modify('+1 year'))
                                ->setOtodokeday((new \DateTime())->modify('+3 year'))
                        )
                        ->setHanpuSecond((new AddHanpuNext\HanpuSecondModel())
                            ->setSite(1)
                            // ->setSiteday(21)
                            ->setSdd(3)
                            ->setWeeksite(1)
                        ...
                        ->setWeborderno(1234);
        $mailjyuden = (new AddHanpuNext\MailJyudenModel())
                        ->setTbikou('メール伝票備考')
                        ->setMail('addHanpuNextTest@AceClient.v1.0');
        $detail = (new AddHanpuNext\DetailModel())
                    ->setHanmei([(new AddHanpuNext\HanmeiModel())
                                    ->setGcode(102)
                                    ->setSuu(7)
                                    ->setTanka(1200)
                                    ->setKousin(1)
                                    ->setKsite(1)
                                    ->setTeiki(1)
                                    ->setTaxkbn(0), (new AddHanpuNext\HanmeiModel())
                            ->setGcode(100)
                            ->setSuu(40)
                            ->setTanka(1650)
                            ->setKousin(0)
        ...
        $prm = (new AddHanpuNext\HanpuPrmModel())
                    ->setMember($member)
                    ->setHanden($handen)
                    ->setMailjyuden($mailjyuden)
                    ->setDetail($detail);

        return (new AddHanpuNext\AddHanpuNextRequestModel())
                    ->setId(OverviewMapper::ACE_TEST_SYID)
                    ->setSessId($this->testSessid)
                    ->setPrm($prm);
    }

    public function testRequestAddHanpuNextOK()
    {
        try {
            $addHanpuNextRequest = $this->getAddHanpuNextModel();
            $response = $this->aceClient->makeHanpuService()
                                        ->makeAddHanpuNextMethod()
                                        ->withRequest($addHanpuNextRequest)
                                        ->send();
            if ($response->getStatusCode() === 200) {
                /** @var AddHanpuNextResponseModel $responseObj */
        ...
        $this->assertEquals('addHanpuNextTest@AceClient.v1.0', $mailjyuden->getMail());

        $this->assertEquals('', $message1);
        $this->assertEquals('', $message2);
    }

    public function testSerializeAddHanpuNext()
    {
        $requestModel = $this->getAddHanpuNextModel();
        $serializer = Serializer\SerializerFactory::makeSoapSerializerForTest();
        $serializedData = $serializer->serialize($requestModel);

        $day = (new \DateTime())->modify('+1 year')->format('Ymd');
        $ckigen = (new \DateTime())->modify('+1 year')->format('Ym');
        $sday = (new \DateTime())->modify('+1 year')->format('Ymd');
        $otodokeday = (new \DateTime())->modify('+3 year')->format('Ymd');
            ...
            <addHanpuNext xmlns="{$xmlns}">
                <prm><![CDATA[<?xml version="1.0" encoding="UTF-8"?>
                    ...
                    <handen>
                        ...
                        <dfmemoh1>伝票フリーメモ1</dfmemoh1>
                        <dfmemoh2>伝票フリーメモ2</dfmemoh2>
                        <dfmemoh3>伝票フリーメモ3</dfmemoh3>
                    </handen>
                    <member>
                        <jmember>
                        <code>221</code>
                    ...
                    <mailjyuden>
                        ...
                        <mail>addHanpuNextTest@AceClient.v1.0</mail>
                    </mailjyuden>
                    </hanpu>
                    ]]></prm>
                <id>13</id>
                <sessId>$this->testSessid</sessId>
            </addHanpuNext>
        {$soapEnd}
        XML;
    ...
    public function testRequestAddHanpuNextNG()
    {
        try {
            $addHanpuNextRequest = $this->getAddHanpuNextModel()->setId(-1);
            $response = $this->aceClient->makeHanpuService()
                                        ->makeAddHanpuNextMethod()
                                        ->withRequest($addHanpuNextRequest)
                                        ->send();
