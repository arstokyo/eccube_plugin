...
abstract class AbstractProductRepeatImportCommand extends Command
{
    ...
    /**
     * Get the target import command name to execute
     */
    abstract protected function getTargetImportCommandName(): string;
    ...
    private function executeProductImportCommand(int $creatorId, \DateTime $updateFrom, \DateTime $updateTo, OutputInterface $output, LoggerInterface $logger, int $chunkIndex = 0, int $totalChunks = 1, int $repeatRound = 1): bool
    {
        try {
            // コマンドの引数を準備
            $arguments = [
                'command' => $this->getTargetImportCommandName(),
                'creatorId' => $creatorId,
